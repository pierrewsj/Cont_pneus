...
let medidasExtras = JSON.parse(localStorage.getItem("medidasExtrasPneus") || "[]");
let marcasExtras = JSON.parse(localStorage.getItem("marcasExtrasPneus") || "[]");
let editandoId = null;

const el = id => document.getElementById(id);
const normalizar = texto => texto.trim().replace(/\s+/g, " ");
const chave = item => `${item.medida.toLowerCase()}|${item.marca.toLowerCase()}`;

function preencherDatalists() {
  const medidas = [...new Set([...medidasIniciais, ...medidasExtras, ...lancamentos.map(i => i.medida)])].sort();
  const marcas = [...new Set([...marcasIniciais, ...marcasExtras, ...lancamentos.map(i => i.marca)])].sort();
...
function salvar() {
  ...
  localStorage.setItem("medidasExtrasPneus", JSON.stringify(medidasExtras));
  localStorage.setItem("marcasExtrasPneus", JSON.stringify(marcasExtras));
}
...
function adicionarOuAtualizar() {
  ...
  if (editandoId) {
    ...
    if (idx >= 0) {
      lancamentos[idx] = { ...lancamentos[idx], medida, marca, quantidade };
      toast("Lançamento atualizado.");
    }
  } else {
    const novo = { id: crypto.randomUUID(), medida, marca, quantidade, criadoEm: new Date().toISOString() };
...
function renderizar() {
  ...
  const filtrados = lancamentos.filter(i => `${i.medida} ${i.marca}`.toLowerCase().includes(busca));
...
function gerarPDF() {
  ...
  doc.setFontSize(10);
  doc.text(`Nome: ${responsavel}`, 14, 24);
  ...
  doc.autoTable({
    startY: 50,
    head: [["Medida", "Marca / Modelo", "Quantidade"]],
    body: lancamentos.map(i => [i.medida, i.marca, i.quantidade]),
    styles: { fontSize: 9 },
...
function exportarCSV() {
  ...
  const linhas = [["Medida", "Marca/Modelo", "Quantidade"]];
  lancamentos.forEach(i => linhas.push([i.medida, i.marca, i.quantidade]));
...
function adicionarOpcao(tipo) {
  const inputId = tipo === "medida" ? "novaMedida" : "novaMarca";
  const lista = tipo === "medida" ? medidasExtras : marcasExtras;
  const iniciais = tipo === "medida" ? medidasIniciais : marcasIniciais;
  const valor = normalizar(el(inputId).value);

  if (!valor) {
    toast(tipo === "medida" ? "Digite uma medida para salvar." : "Digite uma marca para salvar.");
    return;
  }

  const jaExiste = [...iniciais, ...lista].some(item => item.toLowerCase() === valor.toLowerCase());
  if (jaExiste) {
    toast(tipo === "medida" ? "Essa medida já está cadastrada." : "Essa marca já está cadastrada.");
    return;
  }
  ...
  preencherDatalists();
  el(inputId).value = "";
  toast(tipo === "medida" ? "Medida adicionada às sugestões." : "Marca adicionada às sugestões.");
}
...
el("btnAdicionarMedida").addEventListener("click", () => adicionarOpcao("medida"));
el("btnAdicionarMarca").addEventListener("click", () => adicionarOpcao("marca"));
